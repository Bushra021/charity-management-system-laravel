                ...
                <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <select id="type-select" name="type" required>
                            <option value="news" {{ old('type', $post->type) == 'news' ? 'selected' : '' }}>خبر</option>
                            <option value="event" {{ old('type', $post->type) == 'event' ? 'selected' : '' }}>فعالية</option>
                            <option value="donation" {{ old('type', $post->type) == 'donation' ? 'selected' : '' }}>تبرع</option>
                        </select>
                        @error('type')
                    ...
                    <div>
                        <label>المحتوى:</label>
                        <textarea name="post" rows="5" required>{{ old('post', $post->post) }}</textarea>
                        @error('post')
                        <div style="color:red;">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div id="media-fields" style="display: none;">
                        ...
                        <div>
                            ...
                            @if($post->photo)
                                <br><img src="{{ asset('storage/' . $post->photo) }}" width="150">
                            @endif
                            @error('photo')
                            <div style="color:red;">{{ $message }}</div>
                            @enderror
                        ...
                        <div>
                            ...
                            <br>أو رابط فيديو:
                            <input type="url" name="video_link" value="{{ old('video_link', filter_var($post->video, FILTER_VALIDATE_URL) ? $post->video : '') }}">
                            @error('video_link')
                            <div style="color:red;">{{ $message }}</div>
                            @enderror

                            @if($post->video && !filter_var($post->video, FILTER_VALIDATE_URL))
                                <br><video width="250" controls src="{{ asset('storage/' . $post->video) }}"></video>
                            @endif
                        </div>
